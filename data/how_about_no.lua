windower.raw_register_event('incoming chunk',function (id,original,modified,is_injected,is_blocked)
    local pref = modified:sub(1,4)
    local data = modified:sub(5)
    
-------------- ACTION PACKET ---------------
    if id == 0x28 and not is_injected and not is_blocked then
        ...
        for i = 1,act.target_count do
            ...
            for n = 1,act.targets[i].action_count do
                ...
                act.targets[i].actions[n].stagger = get_bit_packed(data,offset+21,offset+27)%8 -- Value 8 to 63 will knockback
        ...
        return pref..react
    elseif id == 0x05B and not is_injected and not is_blocked and original:byte(0x11) + original:byte(0x12)*256 + original:byte(0x13)*256^2 + original:byte(0x14)*256^3 == windower.ffxi.get_player().id then
        return true
    end
end)
