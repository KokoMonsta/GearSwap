...
function get_sets()
	...
	send_command('@input /macro book 13;wait 5;input /lockstyleset 7')
...
function get_best_castable_single_cure(spell)
	if player.mp < spell.mp_cost then
		if player.mp >= 88 and spell.mp_cost > 88 then
			cancel_spell()
...
function get_best_castable_aoe_cure(spell)
	if player.sub_job == 'SCH' and player.sub_job_level > 0 then
		if player.mp < spell.mp_cost then
			if player.mp >= 120 and spell.mp_cost > 120 then
				cancel_spell()
...
function self_command(command)
    ...
    elseif command == 'dia' then
        direct_push(stack_spells['Dia III'], 'ma', 'bt', false)
	elseif command == 'debuff' then
		direct_push(stack_spells['Slow II'], 'ma', 'bt', false)
		direct_push(stack_spells['Blind II'], 'ma', 'bt', false)
		direct_push(stack_spells['Paralyze II'], 'ma', 'bt', false)
		direct_push(stack_spells['Addle II'], 'ma', 'bt', false)
...
function handle_ping()
	...
	if check_ready_silent() and player.x == lastx and player.y == lasty then
		...
		if not buffactive['Composure'] and is_ability_ready('Composure')then
			direct_push(gearswap.res.job_abilities[247], 'ja', player.name, false)
		end
		
		if not buffactive['refresh'] and is_spell_ready('Refresh III') then
			direct_push(gearswap.res.spells[894], 'ma', player.name, false)
		end
		
		-- if not buffactive['refresh'] and is_spell_ready('Refresh II') then
			-- direct_push(gearswap.res.spells[473], 'ma', player.name, false)
		-- end
		...
		if player.sub_job == 'COR' and player.sub_job_level > 0 then
			if not buffactive["Corsair's Roll"] and is_ability_ready('Phantom Roll') then
				direct_push(gearswap.res.job_abilities[114], 'ja', player.name, false)
			end
		end
		
		-- if player.sub_job == 'COR' and player.sub_job_level > 0 then
			-- if not buffactive['Samurai Roll'] and is_ability_ready('Phantom Roll') then
				-- direct_push(gearswap.res.job_abilities[109], 'ja', player.name, false)
			-- end
		-- end
