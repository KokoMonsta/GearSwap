...
function get_sets()
	...
	send_command('@input /macro book 14;wait 5;gs c jpdetect;wait 5;input /lockstyleset 23')
end
...
function self_command(command)
    ...
    elseif command == 'cp' then
        roll_1 = 'corsair'
        roll_2 = 'samurai'
    elseif command == 'warsam' then
        roll_1 = 'fighter'
        roll_2 = 'samurai'
    elseif command == 'drksam' then
        roll_1 = 'chaos'
        roll_2 = 'samurai'
...
function handle_ping()
	...
	if check_ready_silent() and player.x == lastx and player.y == lasty then
        ...
        if player.status == 'Engaged' and finished_rolling_1 == true and finished_rolling_2 == true then
            if autows and player.tp >= 1800 and player.equipment.ammo == 'empty' then
                add_to_front(stack_weapon_skills['Savage Blade'], 't', {})
            elseif autows and player.tp >= 1000 and player.equipment.ammo ~= 'empty' then
				-- add_to_front(stack_weapon_skills['Last Stand'], 't', {})
				add_to_front(stack_weapon_skills['Leaden Salute'], 't', {})
